...
// Filters by sorting pixels in a 3x3 neighborhood
// and finding the minimum of the pixels
...
cv::Mat custom_filter(const cv::Mat& img)
{
    ...
    for (int i = 1; i < padded.rows - 1; i++) {
        ...
        for (int j = 1; j < padded.cols - 1; j++) {
            ...
            // Assign new pixel value as minimum value in neighborhood
            out.at<uchar>(i - 1, j - 1) = v(0);
...
// Basic box filter, sets the output pixel as mean of pixel values of
// the 3x3 neighborhood
cv::Mat box_filter(const cv::Mat& img)
{
    // Pad original with border
    cv::Mat padded;
    cv::copyMakeBorder(img, padded, 1, 1, 1, 1, cv::BORDER_REPLICATE);
    ...
    for (int i = 1; i < padded.rows - 1; i++) {
        ...
        for (int j = 1; j < padded.cols - 1; j++) {
            ...
            // Assign new pixel value - the mean of the 3x3 neighborhood
            out.at<uchar>(i - 1, j - 1) = (v(0)+v(1)+v(2)+v(3)+v(4)+v(5)+v(6)+v(7)+v(8))/9;
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    cv::Mat my_filtered_0 = custom_filter(img); // Apply custom_filter
    cv::imshow("Image 2: Result after applying a 3x3 min filter", my_filtered_0 );

    cv::Mat my_filtered = box_filter(box_filter(box_filter(box_filter(box_filter(my_filtered_0))))); // Apply box_filter
    cv::imshow("Image 3: Result after applying a 3x3 averaging filter 5 times", my_filtered );

    cv::Mat histogram1 = histogram(my_filtered);
    cv::imshow("Histogram of image 3", draw_histogram_image(histogram1));

    cv::Mat my_filtered_1=transform_intensity(my_filtered, -25, 1.09); // Multiply every pixel by 1.09 to increase contrast, and reduce pixel values by 25
    cv::imshow("Image 4: intensity transformation applied to Image 3", my_filtered_1);

    cv::Mat histogram0 = histogram(my_filtered_1 );
    cv::imshow("Histogram of image 4", draw_histogram_image(histogram0));
